...
const FriendHistoryScreen = ({route}) => {
	const [trails, setTrails] = useState([]);
	const friendId = Object.values(route.params)[0];
	const isFocused = useIsFocused();

	useEffect(() => {
		restAPI.getTrails(friendId)
			.then(response => {
				return response.json();
			})
			.then(data => {
...
export default FriendHistoryScreen;
